...
if { [string first $scripts_vivado_version $current_vivado_version] == -1 } {
   puts ""
   catch {puts "ERROR: This script was generated using Vivado <$scripts_vivado_version> and is being run in <$current_vivado_version> of Vivado. Please run the script in Vivado <$scripts_vivado_version> then open the design in Vivado <$current_vivado_version>. Upgrade the design by running \"Tools => Report => Report IP Status...\", then run write_bd_tcl to create an updated script."}
...
"
   ...
   if { $list_ips_missing ne "" } {
      catch {puts "ERROR: The following IPs are not found in the IP Catalog:\n  $list_ips_missing\n\nResolution: Please add the repository containing the IP(s) to the project." }
...
"
   ...
   puts "INFO: Checking if the following modules exist in the project's sources: $list_check_mods ."
   ...
   if { $list_mods_missing ne "" } {
      catch {puts "ERROR: The following module(s) are not found in the project: $list_mods_missing" }
      puts "INFO: Please add source files for the missing module(s) above."
      set bCheckIPsPassed 0
   }
}
...
proc create_hier_cell_IO { parentCell nameHier } {
  ...
  if { $parentCell eq "" || $nameHier eq "" } {
     catch {puts "ERROR: create_hier_cell_IO() - Empty argument(s)!"}
     return
  }
  ...
  if { $parentObj == "" } {
     catch {puts "ERROR: Unable to find parent cell <$parentCell>!"}
     return
  }
  ...
  if { $parentType ne "hier" } {
     catch {puts "ERROR: Parent <$parentObj> has TYPE = <$parentType>. Expected to be <hier>."}
     return
  }
  ...
  if { [catch {set Ethernet [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {puts "ERROR: Unable to add referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   } elseif { $Ethernet eq "" } {
     catch {puts "ERROR: Unable to referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   }
...
proc create_hier_cell_DDR { parentCell nameHier } {
  ...
  if { $parentCell eq "" || $nameHier eq "" } {
     catch {puts "ERROR: create_hier_cell_DDR() - Empty argument(s)!"}
     return
  }
...
proc create_root_design { parentCell } {
  ...
  assign_bd_address -offset 0x60000000 -range 0x00010000 -target_address_space [get_bd_addr_spaces RocketChip/IO_AXI4] [get_bd_addr_segs IO/SD/S_AXI_LITE/reg0] 
  assign_bd_address -offset 0x60010000 -range 0x00010000 -target_address_space [get_bd_addr_spaces RocketChip/IO_AXI4] [get_bd_addr_segs IO/UART/S_AXI_LITE/reg0] 
  assign_bd_address -offset 0x60020000 -range 0x00010000 -target_address_space [get_bd_addr_spaces RocketChip/IO_AXI4] [get_bd_addr_segs IO/Ethernet/S_AXI_LITE/reg0] 
  assign_bd_address -offset 0x60030000 -range 0x00010000 -target_address_space [get_bd_addr_spaces RocketChip/IO_AXI4] [get_bd_addr_segs IO/XADC/s_axi_lite/Reg] 
  ...
  assign_bd_address -offset 0x00000000 -range $addr_range -target_address_space [get_bd_addr_spaces RocketChip/MEM_AXI4] [get_bd_addr_segs DDR/mig_7series_0/memmap/memaddr] 
  assign_bd_address -offset 0x00000000 -range $addr_range -target_address_space [get_bd_addr_spaces IO/Ethernet/M_AXI] [get_bd_addr_segs RocketChip/DMA_AXI4/reg0] 
  assign_bd_address -offset 0x00000000 -range $addr_range -target_address_space [get_bd_addr_spaces IO/SD/M_AXI] [get_bd_addr_segs RocketChip/DMA_AXI4/reg0] 
